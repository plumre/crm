...
layui.config({base : "assets/"}).extend({"goods_config":"admin/goods/goods-config"})
layui.use(["jquery","table","goods_config","mc_children","mc_constant","mc_layui_component","mc_util","mc_rmp"],function(){
    var $ = layui.jquery
        ,layer = parent.layer === undefined ? layui.layer : top.layer
        ,table = layui.table
        ,goods_config = layui.goods_config
        ,mc_children = layui.mc_children
        ,mc_constant = layui.mc_constant
        ,mc_util = layui.mc_util
        ,mc_rmp = layui.mc_rmp
        ,mc_layui_component = layui.mc_layui_component
    ;
    var common_config = {
        ...
        form:{
                price:{
                renderConfig:{}//您可以自定义个性配置 遇到相同的则以dom配置为准 大部分属性已推荐设置在元素标签上 这里您可以自定义事件 {"input":false,"min":0,"max":1000,"showstep":false,"range":false,"inputType":"slider","step":1,"theme":"#009688","type":"default","tips":true,"height":200}
                }
                    ,storeCount:{
                renderConfig:{}//您可以自定义个性配置 遇到相同的则以dom配置为准 大部分属性已推荐设置在元素标签上 这里您可以自定义事件 {"half":false,"readonly":false,"length":6,"inputType":"rate","theme":"0xFFB800","text":false}
                }
                    ,createTime:{
                ...
                renderConfig:{}//您可以自定义个性配置 遇到相同的则以dom配置为准 大部分属性已推荐设置在元素标签上 这里您可以自定义事件 {"calendar":false,"format":"yyyy-MM-dd HH:mm:ss","show":false,"range":false,"trigger":"click","type":"datetime","isInitValue":true,"showBottom":true,"inputType":"date","theme":"default","position":"absolute","lang":"cn","zIndex":66666666}
                    ...
                    ,imgSrc:{
                renderConfig:{}//您可以自定义个性配置 遇到相同的则以dom配置为准 大部分属性已推荐设置在元素标签上 这里您可以自定义事件 {"auto":true,"acceptMime":"images","multiple":false,"inputType":"imgfile","drag":true,"exts":"jpg|png|gif|bmp|jpeg","accept":"images"}
                }
        }
    }
//后端请求时候的表模块规则url
...
//
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
            ...
            {field: 'goodsName', title: '商品名', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.goodsName) +'" class="magicalcoder-table-text layui-input security_list_table_form_goodsName" name="goodsName" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="商品名"/>'
                }
                , sort:true
            },
            {field: 'publishStatus', title: '是否发布',minWidth:350, align:'center', templet:'#publishStatusTemplate',sort:true},

                {field: 'goodsStatus', title: '商品状态',minWidth:150, align:'center', templet:function(d){
                    var checked = (d.goodsStatus+''=='true' || d.goodsStatus+'' == '1') ? 'checked' : '';
                    return '<input type="checkbox" class="magicalcoder-table-layswitch security_list_table_form_goodsStatus" data-identify="'+d.id+'" name="goodsStatus"  lay-filter="goodsStatusFilter" lay-skin="switch" lay-text="货品|商品"  '+checked+' lay-verify="magicalCoderVerify" magicalcoder-verify="" />'
                    }
                    ,sort:true
                },

            {field: 'price', title: '价格', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.price) +'" class="magicalcoder-table-text layui-input security_list_table_form_price" name="price" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="价格"/>'
                }
                , sort:true
            },

            {field: 'storeCount', title: '库存', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.storeCount) +'" class="magicalcoder-table-text layui-input security_list_table_form_storeCount" name="storeCount" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="库存"/>'
                }
                , sort:true
            },
            ...
            {field: 'imgSrc', title: '图片', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.imgSrc) +'" class="magicalcoder-table-text layui-input security_list_table_form_imgSrc" name="imgSrc" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="图片"/>'
                }
                , sort:true
            },
                {field: 'goodsCategoryId', title: '所属类目', align:'center', minWidth:250, templet:function (d) {
                    ...
                    return '<select class="magicalcoder-table-foreign-select2 layui-input security_list_table_form_goodsCategoryId" lay-ignore="true"  name="goodsCategoryId" data-identify="'+d.id+'" data-value="'+value+'" data-url="admin/goods_category_rest/search" data-id="id" data-text-fields="name" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="所属类目"  data-limit="20">'+option+'</select>'
                    },sort:true
                },
    ...
    var obj = {
                ...
                initTable : function () {
                    ...
                    table.render({//这里是layui的table分页写法 具体可参考layui官方文档
                        ...
                        where : this.buildQueryFormParams(""),//构造查询参数 注意这里layui会缓存你的上一次查询参数 无法清除 只能考虑设置成空字符才能清理
                            ...
                            if(res.flag){
                                mc_rmp.paintBody(moduleName,function () {//权限判断
                                    //禁用一些外键字段 当更多信息场景时才会触发
                                    mc_children.disabledTableParentArea();
        //laytable中有很多控件需要初始化 比如日期等
                                    mc_layui_component.bindMagicalCoderLayUiComponentFromTable(common_config,function(){_this.refreshTableFromCurrentPage()})
        //每一行的操作条目事件
        ...
        render:function () {//入口
            ...
            mc_layui_component.bindMagicalCoderLayUiComponentFromQuery(common_config);//查询区域绘制一下控件
            this.bindAreaEvents();
            if(needInitSelect2){//更多信息入口进来 出现了异步操作 暂无法保证前后顺序一致
            setTimeout(function () {
                    _t.layTable().render();
                },800)
            }else {
