...
layui.config({base : "assets/"}).extend({"goodsfile_config":"admin/goodsfile/goodsfile-config"})
layui.use(["jquery","table","goodsfile_config","mc_children","mc_constant","mc_layui_component","mc_util","mc_rmp"],function(){
    var $ = layui.jquery
        ,layer = parent.layer === undefined ? layui.layer : top.layer
        ,table = layui.table
        ,goodsfile_config = layui.goodsfile_config
...
//
    var CONFIG = $.extend(layui.goodsfile_config,{
        ...
        //排序跟数据库实际字段名的映射
        sortMap:{
            fileId:'file_id',goodsId:'goods_id',fileSrc:'file_src'
        }
    });
    //表格每一列配置
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
                ...
                {field: 'goodsId', title: '所属商品', align:'center', minWidth:250, templet:function (d) {
                    ...
                    return '<select class="magicalcoder-table-foreign-select2 layui-input security_list_table_form_goodsId" lay-ignore="true"  name="goodsId" data-identify="'+d.fileId+'" data-value="'+value+'" data-url="admin/goods_rest/search" data-id="id" data-text-fields="goodsName,imgSrc" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入所属商品"  data-limit="20">'+option+'</select>'
                    },sort:true
                },

            {field: 'fileSrc', title: '文件地址', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.fileSrc) +'" class="magicalcoder-table-text layui-input security_list_table_form_fileSrc" name="fileSrc" data-identify="'+d.fileId+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="file_src"/>'
                }
                , sort:true
            },
