...
layui.config({base : "assets/"}).extend({"goodsimg_config":"admin/goodsimg/goodsimg-config"})
layui.use(["jquery","table","goodsimg_config","mc_children","mc_constant","mc_layui_component","mc_util","mc_rmp"],function(){
    var $ = layui.jquery
        ,layer = parent.layer === undefined ? layui.layer : top.layer
        ,table = layui.table
        ,goodsimg_config = layui.goodsimg_config
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
            ...
            {field: 'imgSrc', title: '图片地址', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.imgSrc) +'" class="magicalcoder-table-text layui-input security_list_table_form_imgSrc" name="imgSrc" data-identify="'+d.imgId+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入图片地址"/>'
                }
                , sort:true
            },
                {field: 'goodsId', title: '所属商品', align:'center', minWidth:250, templet:function (d) {
                    ...
                    return '<select class="magicalcoder-table-foreign-select2 layui-input security_list_table_form_goodsId" lay-ignore="true"  name="goodsId" data-identify="'+d.imgId+'" data-value="'+value+'" data-url="admin/goods_rest/search" data-id="id" data-text-fields="goodsName,imgSrc" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入所属商品"  data-limit="20">'+option+'</select>'
                    },sort:true
                },
