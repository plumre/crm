...
layui.config({base : "assets/"}).extend({"goodscategory_config":"admin/goodscategory/goodscategory-config"})
layui.use(["jquery","table","goodscategory_config","mc_children","mc_constant","mc_layui_component","mc_util","mc_rmp"],function(){
    var $ = layui.jquery
        ,layer = parent.layer === undefined ? layui.layer : top.layer
        ,table = layui.table
        ,goodscategory_config = layui.goodscategory_config
    ...
    var common_config = {
        ...
        form:{
                name:{
                renderConfig:{}//您可以自定义个性配置 遇到相同的则以dom配置为准 大部分属性已推荐设置在元素标签上 这里您可以自定义事件 {"input":false,"min":0,"max":100,"showstep":false,"range":true,"inputType":"date","step":1,"theme":"#1a4d1aff","type":"default","tips":true,"height":200}
                }
        }
    }
//后端请求时候的表模块规则url
...
//
    ...
    var COLS = [[
        {type: "checkbox", /*fixed:"left",*/ width:50},//全选
            ...
            {field: 'name', title: '分类名称', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.name) +'" class="magicalcoder-table-text layui-input security_list_table_form_name" name="name" data-identify="'+d.id+'" lay-verify="magicalCoderVerify|mc_required" magicalcoder-verify=""  placeholder="请输入分类名称"/>'
                }
                , sort:true
            },

            {field: 'keyword', title: '分类另一个关键词', minWidth:200,templet:function (d) {
                    return '<input type="text" value="'+ mc_util.escapeHTML(d.keyword) +'" class="magicalcoder-table-text layui-input security_list_table_form_keyword" name="keyword" data-identify="'+d.id+'" lay-verify="magicalCoderVerify" magicalcoder-verify=""  placeholder="请输入分类另一个关键词"/>'
                }
                , sort:true
            },
